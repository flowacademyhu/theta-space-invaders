...
const { MOTHERSHIPCHAR, EMPTYCHAR, UFOCHAR, MOTHERSHIPCYCLE } = require('./../constants');
  ...
  if (ufo1.exist === false) {
    ...
    if (y > 7) {
      for (let i = (UFOPOS.i + ((ufoArr.length - 1) * 2)); i >= UFOPOS.i; i--) {
        if (matrix[i][UFOPOS.j + (x * 2)] === UFOCHAR) {
...
const moveUfo = (ufoArr, matrix) => {
  ...
  if (UFOPOS.dir === 0) {
    ...
    if (UFOPOS.j + ((ufoArr[0].length - 1) * 2) !== matrix[0].length - 1) {
  ...
  for (let i = 0; i <= UFOPOS.j + ((ufoArr[0].length - 1) * 2); i++) {
    if (matrix[UFOPOS.i + ((ufoArr.length - 1) * 2)][i] === UFOCHAR) {
      stop++;
    }
  }
  if (stop === 0) {
