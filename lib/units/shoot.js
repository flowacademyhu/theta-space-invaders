const { BOARDHEIGHT, EMPTYCHAR, BULLETCHAR, MOTHERSHIPCHAR } = require('../constants');
...
const checkIfHits = (matrix) => {
  if (bulletObj.row > 0) {
    if (bulletObj.exists === true && matrix[bulletObj.row][bulletObj.position] !== EMPTYCHAR) {
      if (matrix[bulletObj.row][bulletObj.position] === MOTHERSHIPCHAR) mothershipGotHit(matrix);
      resetBullet(matrix);
    }
  }
};
