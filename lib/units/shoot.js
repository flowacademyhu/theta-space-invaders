const { BOARDHEIGHT, EMPTYCHAR, BULLETCHAR, MOTHERSHIPCHAR, BUNKERCHAR, POINTS, PLAYER } = require('../constants');
...
const increasePlayerPoints = (str) => {
  if (str === 'mothership') PLAYER.points += POINTS.mothership;
};
...
const putBulletinmatrix = (matrix) => {
  ...
  if (bulletObj.exists === true && bulletObj.row >= 0) {
    ...
    if (bulletObj.row < BOARDHEIGHT - 2 && matrix[bulletObj.row + 1][bulletObj.position] === BULLETCHAR) matrix[bulletObj.row + 1][bulletObj.position] = EMPTYCHAR;
    bulletObj.row--;
  } else {
    resetBullet(matrix);
  }
