const { BOARDHEIGHT, EMPTYCHAR, BULLETCHAR, MOTHERSHIPCHAR, BUNKERCHAR, POINTS } = require('../constants');
const { mothershipGotHit } = require('./mothership');
const { playerObj } = require('./canon');

const bulletObj = { position: 0, row: BOARDHEIGHT - 2, exists: false };

const resetBullet = (matrix) => {
  if (bulletObj.row < BOARDHEIGHT - 2) matrix[bulletObj.row + 1][bulletObj.position] = EMPTYCHAR;
  bulletObj.exists = false;
  bulletObj.row = BOARDHEIGHT - 2;
};

const checkIfHits = (matrix) => {
  if (bulletObj.row >= 0) {
    if (bulletObj.exists === true && matrix[bulletObj.row][bulletObj.position] !== EMPTYCHAR) {
      if (matrix[bulletObj.row][bulletObj.position] === MOTHERSHIPCHAR) {
        mothershipGotHit(matrix);
        playerObj.points += POINTS.mothership;
      }
      if (matrix[bulletObj.row][bulletObj.position] === BUNKERCHAR) bunkerGotHit(matrix);
      resetBullet(matrix);
    }
  }
};

const putBulletinmatrix = (matrix) => {
  checkIfHits(matrix);
  if (bulletObj.exists === true && bulletObj.row >= 0) {
    matrix[bulletObj.row][bulletObj.position] = BULLETCHAR;
    if (bulletObj.row < BOARDHEIGHT - 2) matrix[bulletObj.row + 1][bulletObj.position] = EMPTYCHAR;
    bulletObj.row--;
  } else {
    resetBullet(matrix);
  }
};

const bunkerGotHit = (matrix) => {
  matrix[bulletObj.row][bulletObj.position] = EMPTYCHAR;
};

module.exports = {
  putBulletinmatrix,
  bulletObj
};
